...
var MAC={
	...
	'History': {
		...
		'List': function($id){
			...
			var ret = this.Create($id);

			if($('#HISTORY').length>0){

				$('#HISTORY').html(ret);

				$(".history-down").html(ret);

			}else{

				document.write('<div id="HISTORY" style="width:180px">'+ret+'</div>');
		...
		'Create': function(){
			...
			c = 1;
				...
				for($i=0; $i<jsondata.length; $i++){
					...
					if(typeof jsondata[$i].partlink!='undefined' && jsondata[$i].partlink){

						pa = "<a class='partlink' href='"+jsondata[$i].partlink+"' target='_self'>"+jsondata[$i].partname+"<i class=\"ico libg\"></i></a>";
				...
				img_li += "<li>"+pa+"<a href=\"" + jsondata[$i].link + "\" target=\"_self\"><i class=\"num\">"+c+".</i>" + jsondata[$i].name + "</a></li>";
			...
			r= "<div id=\"mh-box\" class=\"clearfix\"><div id=\"mh-title\" class=\"clearfix\">\u6211\u7684\u89C2\u770B\u5386\u53F2</div><div id=\"mh-ul\"><ul class=\"clearfix\">" + img_li + "</ul></div></div>";

			return r;

		},	

		'Insert': function(name,link,partname,partlink,pic){
			...
			if(jsondata != undefined){
				...
				jsonstr = '{video:[{"name":"'+name+'","link":"'+link+'","partname":"'+partname+'","partlink":"'+partlink+'","pic":"'+pic+'"},';

				for($i=0; $i<=this.Limit; $i++){

					if(this.Json[$i] && this.Json[$i].link!=link){

						jsonstr += '{"name":"'+this.Json[$i].name+'","link":"'+this.Json[$i].link+'","partname":"'+this.Json[$i].partname+'","partlink":"'+this.Json[$i].partlink+'","pic":"'+this.Json[$i].pic+'"},';
	...
	'Score': {
		...
		'Show':function($f,$tab,$id){
			...
			this.alt=['很差','较差','还行','推荐','力荐'],this.src=[SitePath+'images/star0.gif',SitePath+'images/star1.gif'];

			this.$id = $id,this.l=5,this.x=0,this.y=0,this.n=0;
			...
			if($f==1){
				...
				str += '&nbsp;<input type="button" value=" 评分 " id="scoresend" style="width:55px;height:21px"/></div></div>';
				...
				for(var i=1;i<=l;i++){
					...
					document.write('<b class="vpf2" onmouseover=\"this.className=\'vpf2hover\'\" onmouseout=\"this.className=\'vpf2\'\"><img id="'+i+'" src="'+MAC.Score.src[1]+'" style="cursor:pointer"><i class="valt">'+MAC.Score.alt[parseInt(i/l*(MAC.Score.alt.length-1))]+'<i class="vsj"></i></i></b>');
			...
			var dc=document.getElementById($id),im=dc.getElementsByTagName('img');this.im = im;
			...
			for(var i=0;i<im.length;i++){
				...
				im[i].onmouseover=function (){
					...
					for(var i=0;i<im.length;i++) im[i].src=MAC.Score.src[x>=parseInt(im[i].id) ? 0 : 1];
			...
			dc.onmouseout=function (){
				...
				for(var i=0;i<im.length;i++) im[i].src=MAC.Score.src[MAC.Score.x>=parseInt(im[i].id) ? 0 : 1];

				document.getElementById('MARK_B2').innerHTML=MAC.Score.y;

				$('#MARK_B3').text(MAC.Score.y>0 ?MAC.Score.n+'次评分' : '请选择') ;
					...
					if($f==1){
						...
						for(var i=0;i<im.length;i++){
							...
							im[i].onclick=function (){
								...
								MAC.Score.Send( '&tab='+$tab+'&id='+$id+'&score='+x*2);
		...
		'View':function($r){
			...
			MAC.Score.y = parseInt($r.score/2*10)/10,MAC.Score.x=parseInt(MAC.Score.y),MAC.Score.n=parseInt($r.scorenum);

			$('#MARK_B2').text(MAC.Score.y);

			$('#MARK_B3').text(MAC.Score.y>0 ?MAC.Score.n+'次评分' : '请选择') ;

			for(var i=0;i<MAC.Score.im.length;i++) MAC.Score.im[i].src=MAC.Score.src[MAC.Score.x>=parseInt(MAC.Score.im[i].id) ? 0 : 1];
	...
	'Digg': {
		...
		'View': function ($tab,$r){
			...
			if($tab == 'vod'){
				...
				$(".digg_vodup>span,#digg_num").html($r.split(':')[0]);

				$(".digg_voddown>span,#tread_num").html($r.split(':')[1]);
